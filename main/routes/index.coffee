...
exports.blog_all = (req, res) ->
  ...
  blog_keys = _.keys blogs_object

  #This page exists?
  if !blog_keys[index * 3]?
    render_views(req, res, 'view_404', {}, 404)
    return

  entries = {}
  for num in [0..2] by 1
    if blogs_object[blog_keys[index * 3 + num]]?
      entries[blog_keys[index * 3 + num]] = blogs_object[blog_keys[index * 3 + num]]

  methods = []
  for key of entries
    ((key, res, entries) ->
      methods.push (callback) ->
        res.render 'blogs/view_blogs_' + key, entries: entries, (err, html) ->  
          entries[key].body = html      
          callback null, key
    )(key, res, entries)

  async.parallel methods, (err, results) ->
    render_views(req, res, 'view_blog_all', entries: entries)


exports.blog_entry = (req, res) ->
  if blogs_object[req.params.entry]?
    res.render 'blogs/view_blogs_' + req.params.entry, {}, (err, html) ->
      render_views(req, res, 'view_blog_post', _.extend blogs_object[req.params.entry], html: html )
  else 
    render_views(req, res, 'view_404', {}, 404)
